...
import { 
  Calendar, User, Clock, Monitor, MousePointer, 
  Image as ImageIcon, X, FileText, Activity, CheckCircle, AlertCircle, Info 
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-6 border-b border-slate-200">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              Employee Detailed Activity
            </h1>
            <p className="mt-1 text-slate-500">Monitor employee attendance timelines, idle times, screenshots, and daily justifications.</p>
          </div>
        </div>

        {/* Controls Panel */}
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-8 border border-slate-150">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Select Employee</label>
              <div className="relative">
                <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400" size={18} />
                <select
                  value={selectedEmployee}
                  onChange={(e) => setSelectedEmployee(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all font-semibold text-slate-700 bg-slate-50/50 cursor-pointer"
                >
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-400 bg-white rounded-2xl border border-dashed border-slate-200 shadow-sm">
            <User className="mx-auto text-slate-300 mb-4 w-12 h-12" />
            <p className="font-semibold text-slate-500">Please select an employee and date to display their timeline details.</p>
          </div>
        ) : (
          <div className="space-y-8">
            ...
            {dailySummary ? (
              ...
              <div className="bg-amber-50 border border-amber-200 text-amber-800 px-6 py-4 rounded-2xl flex items-center gap-3">
                <AlertCircle />
                <span className="font-medium">No attendance record found for this employee on this date.</span>
              </div>
            )}

            {/* Daily Justification Note & Inactive Gaps Layout */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              
              {/* Daily Justification Note (Read-Only for Admin) */}
              <div className="bg-white p-6 rounded-2xl border border-slate-150 shadow-sm flex flex-col justify-between lg:col-span-1">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mb-4">Note added by employee to justify their idle or untracked durations.</p>
                  
                  {dailySummary?.daily_note ? (
                    <div className="p-4 bg-indigo-50/50 border border-indigo-100 text-slate-700 text-sm font-medium rounded-xl leading-relaxed whitespace-pre-line min-h-[110px]">
                      "{dailySummary.daily_note}"
                    </div>
                  ) : (
                    <div className="text-center py-8 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-150 min-h-[110px] flex flex-col justify-center items-center">
                      <Info className="text-slate-300 mb-1 w-5 h-5" />
                      <span className="text-xs">No justification note submitted for this date.</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-150 shadow-sm lg:col-span-2">
                <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2 mb-2">
                  ...
                  Away & Untracked Time Periods
                </h3>
                <p className="text-xs text-slate-400 mb-4 font-normal">Lists exact windows when the tracking was stopped or employee went idle.</p>
                ...
                <div className="space-y-3 overflow-y-auto max-h-56 pr-2">
                  {inactivePeriods.length > 0 ? (
                    ...
                    <div className="text-center py-10 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-150">
                      ...
                      <p className="text-sm font-medium">No idle or untracked gaps found for today!</p>
                    </div>
                  )}
                </div>
              </div>
                    ...
                    {screenshots.map((screenshot) => (
                      ...
                      >
                        ...
                        <div className="p-4 bg-white border-t border-slate-100">
                          <p className="text-xs font-bold text-indigo-600 mb-1">{formatTime(screenshot.timestamp)}</p>
                          <p className="text-sm font-semibold text-slate-800 truncate" title={screenshot.active_window_title}>
                            {screenshot.active_window_title || 'Unknown Window'}
                          </p>
                        </div>
                      </div>
                    ))}
